...
class Blued(IMDB):
    ...
    def gt_roidb(self):
        ...
        print('roidb len: {}'.format(len(gt_roidb)))
        self.num_images = len(gt_roidb)
        self.fixed_image_set_index = _get_list_inter(self.image_set_index, [x['image'] for x in gt_roidb])
    ...
    def do_python_eval(self):
        ...
        table = PrettyTable(['Class Index','Class Name','AP(%)','IoU'])
        ovthresh = 0.5
        ...
        # logger.info('Mean AP = {:.4f}'.format(np.mean(aps)))
            ...
            table.add_row([cls_ind, cls, '{:.4f}'.format(100*ap), ovthresh])
