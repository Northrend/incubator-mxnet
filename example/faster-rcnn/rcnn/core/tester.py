...
def pred_eval(predictor, test_data, imdb, vis=False, thresh=1e-3):
    ...
    if os.path.isfile(det_file):
        print('loading detection results: {}'.format(det_file))
        ...
        print('pred_eval num images: {}'.format(num_images))
        # all detections are collected into:
        #    all_boxes[cls][image] = N x 5 array of detections in
        #    (x1, y1, x2, y2, score)
        all_boxes = [[[] for _ in xrange(num_images)]
                     for _ in xrange(imdb.num_classes)]
        # all_boxes = [[[]*num_images]*imdb.num_classes]    
        ...
        for im_info, data_batch in test_data:
            ...
            t3 = time.time() - t
            t = time.time()
            print('testing {}/{} data {:.4f}s net {:.4f}s post {:.4f}s'.format(i+1, imdb.num_images, t1, t2, t3))
            i += 1
